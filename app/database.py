...
# PUT bookmark 즐겨찾기 (상태 반전)
async def toggle_bookmark_like(bookmarkId: PydanticObjectId):
    bookmark = await Bookmark.get(bookmarkId)

    if not bookmark:
        return None
    
    new_like = not bookmark.like #상태 반전
    
    await bookmark.set({
        Bookmark.like: new_like
    })
